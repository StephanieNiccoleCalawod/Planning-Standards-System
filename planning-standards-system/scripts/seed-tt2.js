    ...
    for (const kpi of templates) {
      try {
        ...
        if (err.status === 409) {
          ...
          // Try to find it for commitment references
          try {
            ...
            if (match) createdKpis.push(match);
          } catch (_) { /* best effort */ }
        } else {
...
async function main() {
  ...
  // Fetch ALL services once using cross-office
  let allServices = [];
  try {
  ...
  // Group by their actual office names stored in the DB (e.g. 'Campus Administrative Office')
  ...
  // Pick up to N services per office
  ...
  // ── Step 4: Seed commitments (per office, for Q1-2026) ──
