...
export default function KPIStandards() {
    ...
    const handleSave = async (e) => {
        ...
        // Uniqueness validation: reject only an EXACT duplicate —
        // same name + same category + same linked service (matches backend createKpi rule).
        // Same name with a different category, or a different linked service, is allowed.
        // e.g. "Good Moral" (Timeliness) and "Good Moral" (Quality) on the same service = OK.
        if (name.trim() && serviceId) {
            const isExactDuplicate = kpis.some(k =>
                k.active &&
                k.id !== editingKpi?.id &&
            ...
            if (isExactDuplicate) {
                err.name = `A KPI named "${name.trim()}" with category "${category}" already exists for this Service Charter.`;
            }
        }
                                    ...
                                    >
                                        ...
                                        <TableCell>
                                            {(() => {
                                                ...
                                                if (!svc || !val || val.trim() === "" || val.toLowerCase() === getArtaClassification(svc).toLowerCase()) {
