...
export const useAppStore = create((set, get) => ({
    ...
    updateServiceIntakeFieldsLocal: (updatedSvc) => {
        set(state => ({
            services: state.services.map(s => s.id === updatedSvc.id ? { ...s, ...updatedSvc, isNew: false } : s)
        }));
    },
    ...
    fetchServiceModes: async (includeInactive = false) => {
        ...
        try {
            ...
            set({ loadingServiceModes: false });
            console.error('useAppStore.fetchServiceModes failed:', err);
            throw err;
        }
    },
